...
class WorkflowItem(object):
    ...
    def _execute(self, activity, stack):
        ...
        if activity['kind'] == WorkflowActivity.KIND_DUMMY:
            ...
            if self.workitem['state'] == 'active':
                ...
                if activity.get('action', False):
                    ...
                    if not id_new:
                        ...
                        return False
                    assert type(id_new) == type(1) or type(id_new) == type(1L), 'Wrong return value: ' + str(
            ...
            #         self._state_set(activity, 'complete')
            if self.workitem['state'] == 'running':
                if not activity['waiting_subflow_done']:
                    self._state_set(activity, 'complete')
                else:
                    cr.execute("select state from wkf_instance where id=%s", (self.workitem['subflow_id'],))
                    state = cr.fetchone()[0]
        ...
        for instance_id, record, signal_send in signal_todo:
            # michael at subflow end  restart mail_todo  of parent subflow
            # wi = instance.WorkflowInstance(self.session, record, {'id': instance_id})
            # wi.validate(signal_send, force_running=True)   #double run instance 108 line
            # add:
            cr.execute(""" select a.subflow_id,a.state,b.res_type,b.wkf_id,b.res_id,c.waiting_subflow_done,c.name as activity_name
                             from wkf_workitem a left join wkf_instance b on a.inst_id=b.id
                                                 left join wkf_activity c on a.act_id=c.id
                            where b.res_type='{res_type}' and b.res_id={res_id}
            """.format(res_type=record.model, res_id=record.id))
            result = cr.fetchall()
            for subflow_id, state, res_type, wkf_id, res_id, waiting_subflow_done, activity_name in result:
                if subflow_id and waiting_subflow_done and activity_name == signal_send:
                    cr.execute(""" update mail_todo set state='active' where res_model='{res_model}' and res_id={res_id} and state='none'
                    """.format(res_model=record.model, res_id=record.id))
                    break
    ...
    def wkf_expr_check(self, transition, signal):
        ...
        if signal is not None:
            if transition['signal'] and signal != transition['signal']:
                return False
        elif not transition['is_not_button']:  # is_not_button==True not check ,all pass
            return False
        ...
        if self.session.uid != odoo.SUPERUSER_ID and transition['group_id']:
            ...
            if transition['group_id'] not in user_groups.ids:
                return False
        rtn = self.wkf_expr_eval_expr(transition['condition'])
        if rtn and transition['is_not_button']:  # michael
            self.session.cr.execute("""
                                update wkf_instance set signal='{signal}'
                                 where res_id={res_id}
                                   and res_type='{res_type}'
                            """.format(signal=transition['signal'],
                                       res_id=self.record.id,
                                       res_type=self.record.model))
        return rtn
