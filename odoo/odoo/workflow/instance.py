...
class WorkflowInstance(object):
    ...
    def create(cls, session, record, workflow_id):
        ...
        cr = session.cr
        cr.execute('insert into wkf_instance (res_type,res_id,uid,wkf_id,state) values (%s,%s,%s,%s,%s) RETURNING id',
    ...
    def delete(self):
        self.session.cr.execute('delete from wkf_instance where res_id=%s and res_type=%s',
    ...
    def _update_end(self):
        ...
        if ok:
            cr.execute(
                'select distinct a.name from wkf_activity a left join wkf_workitem w on (a.id=w.act_id) where w.inst_id=%s',
                (instance_id,))
            ...
            for cur_instance_id, cur_model_name, cur_record_id in cr.fetchall():
                ...
                for act_name in act_names:
                    # michael
                    # WorkflowInstance(self.session, cur_record, {'id': cur_instance_id}).validate(
                    #     'subflow.%s' % act_name[0])
                    WorkflowInstance(self.session, cur_record, {'id': cur_instance_id}).validate(signal='None')
        return ok
